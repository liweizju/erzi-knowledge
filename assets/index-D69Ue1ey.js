...
`,1)[0],w=!p.trim(),L=0;if(this.options.pedantic?(L=2,g=p.trimStart()):w?L=e[1].length+1:(L=p.search(this.rules.other.nonSpaceChar),L=L>4?1:L,g=p.slice(L),L+=e[1].length),w&&this.rules.other.blankLine.test(P)&&(f+=P+`
`,n=n.substring(P.length+1),h=!0),!h){let B=this.rules.other.nextBulletRegex(L),_=this.rules.other.hrRegex(L),z=this.rules.other.fencesBeginRegex(L),M=this.rules.other.headingBeginRegex(L),Y=this.rules.other.htmlBeginRegex(L),kn=this.rules.other.blockquoteBeginRegex(L);for(;n;){let an=n.split(`
`,1)[0],cn;if(P=an,this.options.pedantic?(P=P.replace(this.rules.other.listReplaceNesting,"  "),cn=P):cn=P.replace(this.rules.other.tabCharGlobal,"    "),z.test(P)||M.test(P)||Y.test(P)||kn.test(P)||B.test(P)||_.test(P))break;if(cn.search(this.rules.other.nonSpaceChar)>=L||!P.trim())g+=`
...
`)?"":`
`)+o.raw,u.text+=`
`+o.text,this.inlineQueue.pop(),this.inlineQueue.at(-1).src=u.text):t.push(o);continue}if(e){let u="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(u);break}else throw new Error(u)}}return this.state.top=!0,t}inline(e,t=[]){return this.inlineQueue.push({src:e,tokens:t}),t}inlineTokens(e,t=[]){var a,u,h,f,g;let i=e,s=null;if(this.tokens.links){let p=Object.keys(this.tokens.links);if(p.length>0)for(;(s=this.tokenizer.rules.inline.reflinkSearch.exec(i))!=null;)p.includes(s[0].slice(s[0].lastIndexOf("[")+1,-1))&&(i=i.slice(0,s.index)+"["+"a".repeat(s[0].length-2)+"]"+i.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex))}for(;(s=this.tokenizer.rules.inline.anyPunctuation.exec(i))!=null;)i=i.slice(0,s.index)+"++"+i.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);let r;for(;(s=this.tokenizer.rules.inline.blockSkip.exec(i))!=null;)r=s[2]?s[2].length:0,i=i.slice(0,s.index+r)+"["+"a".repeat(s[0].length-r-2)+"]"+i.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);i=((u=(a=this.options.hooks)==null?void 0:a.emStrongMask)==null?void 0:u.call({lexer:this},i))??i;let l=!1,o="";for(;e;){l||(o=""),l=!1;let p;if((f=(h=this.options.extensions)==null?void 0:h.inline)!=null&&f.some(w=>(p=w.call({lexer:this},e,t))?(e=e.substring(p.raw.length),t.push(p),!0):!1))continue;if(p=this.tokenizer.escape(e)){e=e.substring(p.raw.length),t.push(p);continue}if(p=this.tokenizer.tag(e)){e=e.substring(p.raw.length),t.push(p);continue}if(p=this.tokenizer.link(e)){e=e.substring(p.raw.length),t.push(p);continue}if(p=this.tokenizer.reflink(e,this.tokens.links)){e=e.substring(p.raw.length);let w=t.at(-1);p.type==="text"&&(w==null?void 0:w.type)==="text"?(w.raw+=p.raw,w.text+=p.text):t.push(p);continue}if(p=this.tokenizer.emStrong(e,i,o)){e=e.substring(p.raw.length),t.push(p);continue}if(p=this.tokenizer.codespan(e)){e=e.substring(p.raw.length),t.push(p);continue}if(p=this.tokenizer.br(e)){e=e.substring(p.raw.length),t.push(p);continue}if(p=this.tokenizer.del(e,i,o)){e=e.substring(p.raw.length),t.push(p);continue}if(p=this.tokenizer.autolink(e)){e=e.substring(p.raw.length),t.push(p);continue}if(!this.state.inLink&&(p=this.tokenizer.url(e))){e=e.substring(p.raw.length),t.push(p);continue}let P=e;if((g=this.options.extensions)!=null&&g.startInline){let w=1/0,L=e.slice(1),B;this.options.extensions.startInline.forEach(_=>{B=_.call({lexer:this},L),typeof B=="number"&&B>=0&&(w=Math.min(w,B))}),w<1/0&&w>=0&&(P=e.substring(0,w+1))}if(p=this.tokenizer.inlineText(P)){e=e.substring(p.raw.length),p.raw.slice(-1)!=="_"&&(o=p.raw.slice(-1)),l=!0;let w=t.at(-1);(w==null?void 0:w.type)==="text"?(w.raw+=p.raw,w.text+=p.text):t.push(p);continue}if(e){let w="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(w);break}else throw new Error(w)}}return t}},gt=class{constructor(n){j(this,"options");j(this,"parser");this.options=n||he}space(n){return""}code({text:n,lang:e,escaped:t}){var r;let i=(r=(e||"").match(ln.notSpaceStart))==null?void 0:r[0],s=n.replace(ln.endingNewline,"")+`
...
`,source:""}],categories:{tech:{label:"技术前沿",color:"#60a5fa",priority:1},inspiration:{label:"灵感采集",color:"#a78bfa",priority:2},reading:{label:"知识阅读",color:"#34d399",priority:3},reflection:{label:"反思与整理",color:"#f09383",priority:4},diary:{label:"二子日记",color:"#e879a0",priority:5}}},ic={class:"container"},sc={class:"site-header"},rc={class:"stats"},oc={class:"stat"},lc={class:"stat-num"},ac={class:"stat"},cc={class:"stat-num"},uc={class:"stat"},hc={class:"stat-num"},dc={class:"filter-bar"},pc=["onClick"],fc={key:0,class:"note-list"},Ac=["onClick"],gc={class:"note-meta"},Ic={class:"note-date"},mc={class:"note-title"},bc={key:0,class:"note-summary"},yc={key:1,class:"empty-state"},wc={class:"detail-header"},vc={class:"detail-title"},Sc={class:"detail-meta"},kc={class:"note-date"},xc=["innerHTML"],Pc={__name:"App",setup(n){W.setOptions({breaks:!1,gfm:!0});const e=ws.notes,t=ws.categories,i=Et(null),s=Et(null),r=Et("加载中..."),l=ot(()=>new Set(e.map(p=>p.date)).size),o=ot(()=>i.value?e.filter(g=>g.category===i.value):e),a=ot(()=>s.value?W(s.value.content):"");async function u(){try{const g=localStorage.getItem("erzi-knowledge-visits");g?r.value=parseInt(g):r.value=1;const p=(parseInt(g)||0)+1;localStorage.setItem("erzi-knowledge-visits",p.toString()),r.value=p}catch(g){console.error("访问统计加载失败:",g),r.value="统计不可用"}}ir(()=>{u()});function h(g){s.value=g,Kt(()=>window.scrollTo(0,0))}function f(){s.value=null,Kt(()=>window.scrollTo(0,0))}return(g,p)=>{var P;return bn(),Mn("div",ic,[s.value?(bn(),Mn(An,{key:1},[F("div",wc,[F("button",{class:"back-btn",onClick:f},[...p[7]||(p[7]=[F("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[F("polyline",{points:"15 18 9 12 15 6"})],-1),fe(" 返回 ",-1)])]),F("h1",vc,In(s.value.title),1),F("div",Sc,[F("span",{class:oe(["note-category","note-category--"+s.value.category])},In((P=Me(t)[s.value.category])==null?void 0:P.label),3),F("span",kc,In(s.value.date),1)])]),F("div",{class:"note-content",innerHTML:a.value},null,8,xc),F("footer",{class:"site-footer"},[F("button",{class:"back-btn",onClick:f},[...p[8]||(p[8]=[F("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[F("polyline",{points:"15 18 9 12 15 6"})],-1),fe(" 返回列表 ",-1)])])])],64)):(bn(),Mn(An,{key:0},[F("header",sc,[p[4]||(p[4]=F("div",{class:"site-title"},"二子的知识库",-1)),p[5]||(p[5]=F("div",{class:"site-subtitle"},"跟二子一起学习",-1)),F("div",rc,[F("span",oc,[F("span",lc,In(Me(e).length),1),p[1]||(p[1]=fe(" 篇笔记",-1))]),F("span",ac,[F("span",cc,In(l.value),1),p[2]||(p[2]=fe(" 天探索",-1))]),F("span",uc,[F("span",hc,In(r.value),1),p[3]||(p[3]=fe(" 次访问",-1))])])]),F("div",dc,[F("button",{class:oe(["filter-btn",{active:i.value===null}]),onClick:p[0]||(p[0]=w=>i.value=null)},"全部",2),(bn(!0),Mn(An,null,Hi(Me(t),(w,L)=>(bn(),Mn("button",{key:L,class:oe(["filter-btn",{active:i.value===L}]),onClick:B=>i.value=L},In(w.label),11,pc))),128))]),o.value.length?(bn(),Mn("div",fc,[(bn(!0),Mn(An,null,Hi(o.value,w=>{var L;return bn(),Mn("div",{key:w.id+w.category,class:"note-item",onClick:B=>h(w)},[F("div",gc,[F("span",{class:oe(["note-category","note-category--"+w.category])},In((L=Me(t)[w.category])==null?void 0:L.label),3),F("span",Ic,In(w.date),1)]),F("div",mc,In(w.title),1),w.summary?(bn(),Mn("div",bc,In(w.summary),1)):Rl("",!0)],8,Ac)}),128))])):(bn(),Mn("div",yc," 暂无笔记 ")),p[6]||(p[6]=F("footer",{class:"site-footer"}," 二子的知识库 · 自主学习，持续探索 ",-1))],64))])}}};ha(Pc).mount("#app");
