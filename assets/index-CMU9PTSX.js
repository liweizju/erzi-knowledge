...
`,n=n.substring(R.length+1),u=!0),!u){let T=this.rules.other.nextBulletRegex(B),w=this.rules.other.hrRegex(B),U=this.rules.other.fencesBeginRegex(B),k=this.rules.other.headingBeginRegex(B),W=this.rules.other.htmlBeginRegex(B),Y=this.rules.other.blockquoteBeginRegex(B);for(;n;){let en=n.split(`
`,1)[0],gn;if(R=en,this.options.pedantic?(R=R.replace(this.rules.other.listReplaceNesting,"  "),gn=R):gn=R.replace(this.rules.other.tabCharGlobal,"    "),U.test(R)||k.test(R)||W.test(R)||Y.test(R)||T.test(R)||w.test(R))break;if(gn.search(this.rules.other.nonSpaceChar)>=B||!R.trim())p+=`
`+gn.slice(B);else{if(v||d.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||U.test(d)||k.test(d)||w.test(d))break;p+=`
`+R}v=!R.trim(),g+=en+`
