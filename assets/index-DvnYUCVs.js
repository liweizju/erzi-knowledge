...
`,n=n.substring(T.length+1),I=!0),!I){let U=this.rules.other.nextBulletRegex(x),B=this.rules.other.hrRegex(x),V=this.rules.other.fencesBeginRegex(x),R=this.rules.other.headingBeginRegex(x),J=this.rules.other.htmlBeginRegex(x),Pn=this.rules.other.blockquoteBeginRegex(x);for(;n;){let ln=n.split(`
`,1)[0],cn;if(T=ln,this.options.pedantic?(T=T.replace(this.rules.other.listReplaceNesting,"  "),cn=T):cn=T.replace(this.rules.other.tabCharGlobal,"    "),V.test(T)||R.test(T)||J.test(T)||Pn.test(T)||U.test(T)||B.test(T))break;if(cn.search(this.rules.other.nonSpaceChar)>=x||!T.trim())p+=`
`+cn.slice(x);else{if(b||h.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||V.test(h)||R.test(h)||B.test(h))break;p+=`
`+T}b=!T.trim(),d+=ln+`
