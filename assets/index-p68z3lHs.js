...
`,n=n.substring(x.length+1),h=!0),!h){let O=this.rules.other.nextBulletRegex(M),_=this.rules.other.hrRegex(M),z=this.rules.other.fencesBeginRegex(M),R=this.rules.other.headingBeginRegex(M),Q=this.rules.other.htmlBeginRegex(M),kn=this.rules.other.blockquoteBeginRegex(M);for(;n;){let ln=n.split(`
`,1)[0],cn;if(x=ln,this.options.pedantic?(x=x.replace(this.rules.other.listReplaceNesting,"  "),cn=x):cn=x.replace(this.rules.other.tabCharGlobal,"    "),z.test(x)||R.test(x)||Q.test(x)||kn.test(x)||O.test(x)||_.test(x))break;if(cn.search(this.rules.other.nonSpaceChar)>=M||!x.trim())g+=`
`+cn.slice(M);else{if(w||d.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||z.test(d)||R.test(d)||_.test(d))break;g+=`
`+x}w=!x.trim(),p+=ln+`
