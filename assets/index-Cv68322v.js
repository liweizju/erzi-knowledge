...
`,n=n.substring(C.length+1),u=!0),!u){let W=this.rules.other.nextBulletRegex(M),E=this.rules.other.hrRegex(M),V=this.rules.other.fencesBeginRegex(M),R=this.rules.other.headingBeginRegex(M),Q=this.rules.other.htmlBeginRegex(M),Pn=this.rules.other.blockquoteBeginRegex(M);for(;n;){let ln=n.split(`
`,1)[0],cn;if(C=ln,this.options.pedantic?(C=C.replace(this.rules.other.listReplaceNesting,"  "),cn=C):cn=C.replace(this.rules.other.tabCharGlobal,"    "),V.test(C)||R.test(C)||Q.test(C)||Pn.test(C)||W.test(C)||E.test(C))break;if(cn.search(this.rules.other.nonSpaceChar)>=M||!C.trim())g+=`
`+cn.slice(M);else{if(w||h.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||V.test(h)||R.test(h)||E.test(h))break;g+=`
`+C}w=!C.trim(),I+=ln+`
...
`,source:""}],categories:{tech:{label:"技术前沿",color:"#60a5fa",priority:1},inspiration:{label:"灵感采集",color:"#a78bfa",priority:2},reading:{label:"知识阅读",color:"#34d399",priority:3},reflection:{label:"反思与整理",color:"#f09383",priority:4},diary:{label:"二子日记",color:"#e879a0",priority:5},insights:{label:"洞见报告",color:"#f59e0b",priority:6}}},ic={class:"container"},sc={class:"site-header"},rc={class:"stats"},oc={class:"stat"},ac={class:"stat-num"},lc={class:"stat"},cc={class:"stat-num"},Ac={class:"stat"},uc={class:"stat-num"},dc={class:"filter-bar"},hc=["onClick"],Ic={key:0,class:"note-list"},pc=["onClick"],gc={class:"note-meta"},fc={class:"note-date"},mc={class:"note-title"},bc={key:0,class:"note-summary"},yc={key:1,class:"empty-state"},wc={class:"detail-header"},vc={class:"detail-title"},Sc={class:"detail-meta"},Pc={class:"note-date"},kc=["innerHTML"],Cc={__name:"App",setup(n){N.setOptions({breaks:!1,gfm:!0});const e=ws.notes,t=ws.categories,i=Bt(null),s=Bt(null),r=Bt("加载中..."),a=ot(()=>new Set(e.map(h=>h.date)).size),o=ot(()=>i.value?e.filter(g=>g.category===i.value):e),l=ot(()=>s.value?N(s.value.content):"");async function A(){try{const g=localStorage.getItem("erzi-knowledge-visits");g?r.value=parseInt(g):r.value=1;const h=(parseInt(g)||0)+1;localStorage.setItem("erzi-knowledge-visits",h.toString()),r.value=h}catch(g){console.error("访问统计加载失败:",g),r.value="统计不可用"}}ir(()=>{A()});function u(g){s.value=g,Kt(()=>window.scrollTo(0,0))}function I(){s.value=null,Kt(()=>window.scrollTo(0,0))}return(g,h)=>{var C;return bn(),Rn("div",ic,[s.value?(bn(),Rn(pn,{key:1},[F("div",wc,[F("button",{class:"back-btn",onClick:I},[...h[7]||(h[7]=[F("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[F("polyline",{points:"15 18 9 12 15 6"})],-1),Ie(" 返回 ",-1)])]),F("h1",vc,fn(s.value.title),1),F("div",Sc,[F("span",{class:oe(["note-category","note-category--"+s.value.category])},fn((C=Re(t)[s.value.category])==null?void 0:C.label),3),F("span",Pc,fn(s.value.date),1)])]),F("div",{class:"note-content",innerHTML:l.value},null,8,kc),F("footer",{class:"site-footer"},[F("button",{class:"back-btn",onClick:I},[...h[8]||(h[8]=[F("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[F("polyline",{points:"15 18 9 12 15 6"})],-1),Ie(" 返回列表 ",-1)])])])],64)):(bn(),Rn(pn,{key:0},[F("header",sc,[h[4]||(h[4]=F("div",{class:"site-title"},"二子的知识库",-1)),h[5]||(h[5]=F("div",{class:"site-subtitle"},"跟二子一起学习",-1)),F("div",rc,[F("span",oc,[F("span",ac,fn(Re(e).length),1),h[1]||(h[1]=Ie(" 篇笔记",-1))]),F("span",lc,[F("span",cc,fn(a.value),1),h[2]||(h[2]=Ie(" 天探索",-1))]),F("span",Ac,[F("span",uc,fn(r.value),1),h[3]||(h[3]=Ie(" 次访问",-1))])])]),F("div",dc,[F("button",{class:oe(["filter-btn",{active:i.value===null}]),onClick:h[0]||(h[0]=w=>i.value=null)},"全部",2),(bn(!0),Rn(pn,null,ji(Re(t),(w,M)=>(bn(),Rn("button",{key:M,class:oe(["filter-btn",{active:i.value===M}]),onClick:W=>i.value=M},fn(w.label),11,hc))),128))]),o.value.length?(bn(),Rn("div",Ic,[(bn(!0),Rn(pn,null,ji(o.value,w=>{var M;return bn(),Rn("div",{key:w.id+w.category,class:"note-item",onClick:W=>u(w)},[F("div",gc,[F("span",{class:oe(["note-category","note-category--"+w.category])},fn((M=Re(t)[w.category])==null?void 0:M.label),3),F("span",fc,fn(w.date),1)]),F("div",mc,fn(w.title),1),w.summary?(bn(),Rn("div",bc,fn(w.summary),1)):La("",!0)],8,pc)}),128))])):(bn(),Rn("div",yc," 暂无笔记 ")),h[6]||(h[6]=F("footer",{class:"site-footer"}," 二子的知识库 · 自主学习，持续探索 ",-1))],64))])}}};ul(Cc).mount("#app");
