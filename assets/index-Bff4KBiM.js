...
`,1)[0],cn;if(C=ln,this.options.pedantic?(C=C.replace(this.rules.other.listReplaceNesting,"  "),cn=C):cn=C.replace(this.rules.other.tabCharGlobal,"    "),j.test(C)||R.test(C)||Q.test(C)||Pn.test(C)||B.test(C)||E.test(C))break;if(cn.search(this.rules.other.nonSpaceChar)>=M||!C.trim())I+=`
`+cn.slice(M);else{if(w||d.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||j.test(d)||R.test(d)||E.test(d))break;I+=`
`+C}w=!C.trim(),p+=ln+`
