...
`+t.join(`
`),y=this.blockquote(P);r[r.length-1]=y,s=s.substring(0,s.length-p.raw.length)+y.raw,i=i.substring(0,i.length-p.text.length)+y.text;break}else if((A==null?void 0:A.type)==="list"){let p=A,P=p.raw+`
`+t.join(`
`),y=this.list(P);r[r.length-1]=y,s=s.substring(0,s.length-A.raw.length)+y.raw,i=i.substring(0,i.length-p.raw.length)+y.raw,t=P.substring(r.at(-1).raw.length).split(`
...
`,1)[0],y=!p.trim(),M=0;if(this.options.pedantic?(M=2,A=p.trimStart()):y?M=e[1].length+1:(M=p.search(this.rules.other.nonSpaceChar),M=M>4?1:M,A=p.slice(M),M+=e[1].length),y&&this.rules.other.blankLine.test(P)&&(d+=P+`
...
`+cn.slice(M);else{if(y||p.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||j.test(p)||R.test(p)||L.test(p))break;A+=`
`+P}y=!P.trim(),d+=an+`
...
`,source:""}],categories:{tech:{label:"技术前沿",color:"#60a5fa",priority:1},inspiration:{label:"灵感采集",color:"#a78bfa",priority:2},reading:{label:"知识阅读",color:"#34d399",priority:3},reflection:{label:"反思与整理",color:"#f09383",priority:4},diary:{label:"二子日记",color:"#e879a0",priority:5}}},sc={class:"container"},ic={class:"site-header"},rc={class:"stats"},oc={class:"stat"},lc={class:"stat-num"},ac={class:"stat"},cc={class:"stat-num"},uc={class:"stat"},hc={class:"stat-num"},fc={class:"filter-bar"},pc=["onClick"],dc={key:0,class:"note-list"},gc=["onClick"],Ac={class:"note-meta"},Ic={class:"note-date"},mc={class:"note-title"},bc={key:0,class:"note-summary"},wc={key:1,class:"empty-state"},yc={class:"detail-header"},vc={class:"detail-title"},Sc={class:"detail-meta"},kc={class:"note-date"},xc=["innerHTML"],Pc={__name:"App",setup(n){$.setOptions({breaks:!1,gfm:!0});const e=yi.notes,t=yi.categories,s=Et(null),i=Et(null),r=Et("加载中..."),l=ot(()=>new Set(e.map(p=>p.date)).size),o=ot(()=>s.value?e.filter(A=>A.category===s.value):e),a=ot(()=>i.value?$(i.value.content):"");async function u(){try{const A=localStorage.getItem("erzi-knowledge-visits");A?r.value=parseInt(A):r.value=1;const p=(parseInt(A)||0)+1;localStorage.setItem("erzi-knowledge-visits",p.toString()),r.value=p}catch(A){console.error("访问统计加载失败:",A),r.value="统计不可用"}}sr(()=>{u()});function h(A){i.value=A,Kt(()=>window.scrollTo(0,0))}function d(){i.value=null,Kt(()=>window.scrollTo(0,0))}return(A,p)=>{var P;return bn(),Rn("div",sc,[i.value?(bn(),Rn(gn,{key:1},[W("div",yc,[W("button",{class:"back-btn",onClick:d},[...p[7]||(p[7]=[W("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[W("polyline",{points:"15 18 9 12 15 6"})],-1),de(" 返回 ",-1)])]),W("h1",vc,In(i.value.title),1),W("div",Sc,[W("span",{class:oe(["note-category","note-category--"+i.value.category])},In((P=Re(t)[i.value.category])==null?void 0:P.label),3),W("span",kc,In(i.value.date),1)])]),W("div",{class:"note-content",innerHTML:a.value},null,8,xc),W("footer",{class:"site-footer"},[W("button",{class:"back-btn",onClick:d},[...p[8]||(p[8]=[W("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[W("polyline",{points:"15 18 9 12 15 6"})],-1),de(" 返回列表 ",-1)])])])],64)):(bn(),Rn(gn,{key:0},[W("header",ic,[p[4]||(p[4]=W("div",{class:"site-title"},"二子的知识库",-1)),p[5]||(p[5]=W("div",{class:"site-subtitle"},"跟二子一起学习",-1)),W("div",rc,[W("span",oc,[W("span",lc,In(Re(e).length),1),p[1]||(p[1]=de(" 篇笔记",-1))]),W("span",ac,[W("span",cc,In(l.value),1),p[2]||(p[2]=de(" 天探索",-1))]),W("span",uc,[W("span",hc,In(r.value),1),p[3]||(p[3]=de(" 次访问",-1))])])]),W("div",fc,[W("button",{class:oe(["filter-btn",{active:s.value===null}]),onClick:p[0]||(p[0]=y=>s.value=null)},"全部",2),(bn(!0),Rn(gn,null,Hs(Re(t),(y,M)=>(bn(),Rn("button",{key:M,class:oe(["filter-btn",{active:s.value===M}]),onClick:B=>s.value=M},In(y.label),11,pc))),128))]),o.value.length?(bn(),Rn("div",dc,[(bn(!0),Rn(gn,null,Hs(o.value,y=>{var M;return bn(),Rn("div",{key:y.id+y.category,class:"note-item",onClick:B=>h(y)},[W("div",Ac,[W("span",{class:oe(["note-category","note-category--"+y.category])},In((M=Re(t)[y.category])==null?void 0:M.label),3),W("span",Ic,In(y.date),1)]),W("div",mc,In(y.title),1),y.summary?(bn(),Rn("div",bc,In(y.summary),1)):Gl("",!0)],8,gc)}),128))])):(bn(),Rn("div",wc," 暂无笔记 ")),p[6]||(p[6]=W("footer",{class:"site-footer"}," 二子的知识库 · 自主学习，持续探索 ",-1))],64))])}}};ha(Pc).mount("#app");
