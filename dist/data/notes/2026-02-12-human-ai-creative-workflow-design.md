...
1. 把「AI时代创意工作流设计：从工具到合作者」改写成一个可测试假设（目标用户、触发场景、预期行为）。
2. 用低保真原型改造一个关键流程，并完成 5 人可用性快速测试。
3. 根据测试结果做保留/迭代/放弃决策，并记录一条明确结论。

## 证据与不确定性

- 已确认：IMD的研究发现了一个反直觉的结论：简单给人AI工具并不能自动提升创造力。多个实证研究显示，人机协作的创意产出不一致，甚至会低于纯人类创作。原因在于：大多数研究假设人类和AI会自然学会如何有效协作，但这是错误的。
- 已确认：ArXiv研究将数字工具对创意的支持分为四个层次：
- 已确认：TechClass的研究数据很有说服力：使用AI增强人类能力的企业，其性能提升是仅使用AI自动化任务的企业的3倍。这不是理论，而是实战验证：
- 不确定：IMD的研究发现了一个反直觉的结论：简单给人AI工具并不能自动提升创造力。多个实证研究显示，人机协作的创意产出不一致，甚至会低于纯人类创作。原因在于：大多数研究假设人类和AI会自然学会如何有效协作，但这是错误的。
- 不确定：我的判断： 这是一个被严重低估的风险。组织在追求AI效率时，必须考虑对员工工作意义感的潜在负面影响。这不是软性的HR问题，而是硬性的生产力问题——无意义的工作会导致员工投入度降低，最终抵消AI带来的效率提升。
