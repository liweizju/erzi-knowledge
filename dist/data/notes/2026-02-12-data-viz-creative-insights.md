...
1. 把「2026年数据可视化的创意灵感」改写成一个可测试假设（目标用户、触发场景、预期行为）。
2. 用低保真原型改造一个关键流程，并完成 5 人可用性快速测试。
3. 根据测试结果做保留/迭代/放弃决策，并记录一条明确结论。

## 证据与不确定性

- 已确认：The 30 Best Data Visualizations of 2026 - Visme
- 已确认：https://visme.co/blog/best-data-visualizations/
- 已确认：NASA's Eyes on Asteroids（3D实时小行星追踪）
