...
> **30秒结论**："分析引擎没有任何原创任何东西的伪装。它可以做任何我们知道如何命令它执行的事情。"
...
- 已确认：文章来源： Aeon
- 已确认：这和 LLM 的"思考"方式完全一样：逆向工程。先研究《大白鲨》或《目击者》成功的机制，然后识别可转移的组件，重组以实现类似的艺术成功。
- 已确认：作者是 Philip Stone（Nielsen，英国图书销售数据公司）。我预期他是对的，因为 LLM 会先来类型写作：
- 不确定：爱荷华工作坊的假设： 一套可学习的规则可以生成像样的文学产品。
