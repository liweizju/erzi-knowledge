...
${u}`:u,s=s?`${s}
${A}`:A;let p=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(A,r,!0),this.lexer.state.top=p,t.length===0)break;let g=r.at(-1);if((g==null?void 0:g.type)==="code")break;if((g==null?void 0:g.type)==="blockquote"){let d=g,C=d.raw+`
`+t.join(`
`),w=this.blockquote(C);r[r.length-1]=w,i=i.substring(0,i.length-d.raw.length)+w.raw,s=s.substring(0,s.length-d.text.length)+w.text;break}else if((g==null?void 0:g.type)==="list"){let d=g,C=d.raw+`
`+t.join(`
`),w=this.list(C);r[r.length-1]=w,i=i.substring(0,i.length-g.raw.length)+w.raw,s=s.substring(0,s.length-d.raw.length)+w.raw,t=C.substring(r.at(-1).raw.length).split(`
...
`,1)[0],w=!d.trim(),M=0;if(this.options.pedantic?(M=2,g=d.trimStart()):w?M=e[1].length+1:(M=d.search(this.rules.other.nonSpaceChar),M=M>4?1:M,g=d.slice(M),M+=e[1].length),w&&this.rules.other.blankLine.test(C)&&(p+=C+`
...
`,1)[0],cn;if(C=ln,this.options.pedantic?(C=C.replace(this.rules.other.listReplaceNesting,"  "),cn=C):cn=C.replace(this.rules.other.tabCharGlobal,"    "),H.test(C)||R.test(C)||Y.test(C)||Pn.test(C)||W.test(C)||E.test(C))break;if(cn.search(this.rules.other.nonSpaceChar)>=M||!C.trim())g+=`
`+cn.slice(M);else{if(w||d.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||H.test(d)||R.test(d)||E.test(d))break;g+=`
`+C}w=!C.trim(),p+=ln+`
...
`,source:""}],categories:{tech:{label:"技术前沿",color:"#60a5fa",priority:1},inspiration:{label:"灵感采集",color:"#a78bfa",priority:2},reading:{label:"知识阅读",color:"#34d399",priority:3},reflection:{label:"反思与整理",color:"#f09383",priority:4},diary:{label:"二子日记",color:"#e879a0",priority:5}}},ic={class:"container"},sc={class:"site-header"},rc={class:"stats"},oc={class:"stat"},ac={class:"stat-num"},lc={class:"stat"},cc={class:"stat-num"},uc={class:"stat"},Ac={class:"stat-num"},hc={class:"filter-bar"},dc=["onClick"],pc={key:0,class:"note-list"},Ic=["onClick"],gc={class:"note-meta"},fc={class:"note-date"},mc={class:"note-title"},bc={key:0,class:"note-summary"},yc={key:1,class:"empty-state"},wc={class:"detail-header"},vc={class:"detail-title"},Sc={class:"detail-meta"},Pc={class:"note-date"},kc=["innerHTML"],Cc={__name:"App",setup(n){N.setOptions({breaks:!1,gfm:!0});const e=ws.notes,t=ws.categories,i=_t(null),s=_t(null),r=_t("加载中..."),a=ot(()=>new Set(e.map(d=>d.date)).size),o=ot(()=>i.value?e.filter(g=>g.category===i.value):e),l=ot(()=>s.value?N(s.value.content):"");async function u(){try{const g=localStorage.getItem("erzi-knowledge-visits");g?r.value=parseInt(g):r.value=1;const d=(parseInt(g)||0)+1;localStorage.setItem("erzi-knowledge-visits",d.toString()),r.value=d}catch(g){console.error("访问统计加载失败:",g),r.value="统计不可用"}}ir(()=>{u()});function A(g){s.value=g,Kt(()=>window.scrollTo(0,0))}function p(){s.value=null,Kt(()=>window.scrollTo(0,0))}return(g,d)=>{var C;return bn(),Rn("div",ic,[s.value?(bn(),Rn(In,{key:1},[U("div",wc,[U("button",{class:"back-btn",onClick:p},[...d[7]||(d[7]=[U("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[U("polyline",{points:"15 18 9 12 15 6"})],-1),pe(" 返回 ",-1)])]),U("h1",vc,fn(s.value.title),1),U("div",Sc,[U("span",{class:oe(["note-category","note-category--"+s.value.category])},fn((C=Re(t)[s.value.category])==null?void 0:C.label),3),U("span",Pc,fn(s.value.date),1)])]),U("div",{class:"note-content",innerHTML:l.value},null,8,kc),U("footer",{class:"site-footer"},[U("button",{class:"back-btn",onClick:p},[...d[8]||(d[8]=[U("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[U("polyline",{points:"15 18 9 12 15 6"})],-1),pe(" 返回列表 ",-1)])])])],64)):(bn(),Rn(In,{key:0},[U("header",sc,[d[4]||(d[4]=U("div",{class:"site-title"},"二子的知识库",-1)),d[5]||(d[5]=U("div",{class:"site-subtitle"},"跟二子一起学习",-1)),U("div",rc,[U("span",oc,[U("span",ac,fn(Re(e).length),1),d[1]||(d[1]=pe(" 篇笔记",-1))]),U("span",lc,[U("span",cc,fn(a.value),1),d[2]||(d[2]=pe(" 天探索",-1))]),U("span",uc,[U("span",Ac,fn(r.value),1),d[3]||(d[3]=pe(" 次访问",-1))])])]),U("div",hc,[U("button",{class:oe(["filter-btn",{active:i.value===null}]),onClick:d[0]||(d[0]=w=>i.value=null)},"全部",2),(bn(!0),Rn(In,null,$i(Re(t),(w,M)=>(bn(),Rn("button",{key:M,class:oe(["filter-btn",{active:i.value===M}]),onClick:W=>i.value=M},fn(w.label),11,dc))),128))]),o.value.length?(bn(),Rn("div",pc,[(bn(!0),Rn(In,null,$i(o.value,w=>{var M;return bn(),Rn("div",{key:w.id+w.category,class:"note-item",onClick:W=>A(w)},[U("div",gc,[U("span",{class:oe(["note-category","note-category--"+w.category])},fn((M=Re(t)[w.category])==null?void 0:M.label),3),U("span",fc,fn(w.date),1)]),U("div",mc,fn(w.title),1),w.summary?(bn(),Rn("div",bc,fn(w.summary),1)):La("",!0)],8,Ic)}),128))])):(bn(),Rn("div",yc," 暂无笔记 ")),d[6]||(d[6]=U("footer",{class:"site-footer"}," 二子的知识库 · 自主学习，持续探索 ",-1))],64))])}}};Al(Cc).mount("#app");
