...
`,n=n.substring(k.length+1),u=!0),!u){let F=this.rules.other.nextBulletRegex(x),B=this.rules.other.hrRegex(x),V=this.rules.other.fencesBeginRegex(x),R=this.rules.other.headingBeginRegex(x),J=this.rules.other.htmlBeginRegex(x),Pn=this.rules.other.blockquoteBeginRegex(x);for(;n;){let ln=n.split(`
`,1)[0],cn;if(k=ln,this.options.pedantic?(k=k.replace(this.rules.other.listReplaceNesting,"  "),cn=k):cn=k.replace(this.rules.other.tabCharGlobal,"    "),V.test(k)||R.test(k)||J.test(k)||Pn.test(k)||F.test(k)||B.test(k))break;if(cn.search(this.rules.other.nonSpaceChar)>=x||!k.trim())p+=`
...
`;return i?'<pre><code class="language-'+Nn(i)+'">'+(t?s:Nn(s,!0))+`</code></pre>
`:"<pre><code>"+(t?s:Nn(s,!0))+`</code></pre>
...
`,source:""}],categories:{tech:{label:"技术前沿",color:"#60a5fa",priority:1},inspiration:{label:"灵感采集",color:"#a78bfa",priority:2},reading:{label:"知识阅读",color:"#34d399",priority:3},reflection:{label:"反思与整理",color:"#f09383",priority:4},diary:{label:"二子日记",color:"#e879a0",priority:5},insights:{label:"洞见报告",color:"#f59e0b",priority:6}}},ic={class:"container"},sc={class:"site-header"},oc={class:"stats"},rc={class:"stat"},ac={class:"stat-num"},lc={class:"stat"},cc={class:"stat-num"},Ac={class:"stat"},uc={class:"stat-num"},Ic={class:"filter-bar"},hc=["onClick"],dc={key:0,class:"note-list"},gc=["onClick"],pc={class:"note-meta"},fc={class:"note-date"},mc={class:"note-title"},yc={key:0,class:"note-summary"},bc={key:1,class:"empty-state"},wc={class:"detail-header"},vc={class:"detail-title"},Sc={class:"detail-meta"},Pc={class:"note-date"},Cc=["innerHTML"],kc={__name:"App",setup(n){_.setOptions({breaks:!1,gfm:!0});const e=ws.notes,t=ws.categories,i=Et(null),s=Et(null),o=Et("加载中..."),a=rt(()=>new Set(e.map(h=>h.date)).size),r=rt(()=>i.value?e.filter(p=>p.category===i.value):e),l=rt(()=>s.value?_(s.value.content):"");async function A(){try{const p=localStorage.getItem("erzi-knowledge-visits");p?o.value=parseInt(p):o.value=1;const h=(parseInt(p)||0)+1;localStorage.setItem("erzi-knowledge-visits",h.toString()),o.value=h}catch(p){console.error("访问统计加载失败:",p),o.value="统计不可用"}}io(()=>{A()});function u(p){s.value=p,jt(()=>window.scrollTo(0,0))}function d(){s.value=null,jt(()=>window.scrollTo(0,0))}return(p,h)=>{var k;return yn(),Rn("div",ic,[s.value?(yn(),Rn(gn,{key:1},[N("div",wc,[N("button",{class:"back-btn",onClick:d},[...h[7]||(h[7]=[N("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[N("polyline",{points:"15 18 9 12 15 6"})],-1),de(" 返回 ",-1)])]),N("h1",vc,fn(s.value.title),1),N("div",Sc,[N("span",{class:re(["note-category","note-category--"+s.value.category])},fn((k=Re(t)[s.value.category])==null?void 0:k.label),3),N("span",Pc,fn(s.value.date),1)])]),N("div",{class:"note-content",innerHTML:l.value},null,8,Cc),N("footer",{class:"site-footer"},[N("button",{class:"back-btn",onClick:d},[...h[8]||(h[8]=[N("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[N("polyline",{points:"15 18 9 12 15 6"})],-1),de(" 返回列表 ",-1)])])])],64)):(yn(),Rn(gn,{key:0},[N("header",sc,[h[4]||(h[4]=N("div",{class:"site-title"},"二子的知识库",-1)),h[5]||(h[5]=N("div",{class:"site-subtitle"},"跟二子一起学习",-1)),N("div",oc,[N("span",rc,[N("span",ac,fn(Re(e).length),1),h[1]||(h[1]=de(" 篇笔记",-1))]),N("span",lc,[N("span",cc,fn(a.value),1),h[2]||(h[2]=de(" 天探索",-1))]),N("span",Ac,[N("span",uc,fn(o.value),1),h[3]||(h[3]=de(" 次访问",-1))])])]),N("div",Ic,[N("button",{class:re(["filter-btn",{active:i.value===null}]),onClick:h[0]||(h[0]=w=>i.value=null)},"全部",2),(yn(!0),Rn(gn,null,Ki(Re(t),(w,x)=>(yn(),Rn("button",{key:x,class:re(["filter-btn",{active:i.value===x}]),onClick:F=>i.value=x},fn(w.label),11,hc))),128))]),r.value.length?(yn(),Rn("div",dc,[(yn(!0),Rn(gn,null,Ki(r.value,w=>{var x;return yn(),Rn("div",{key:w.id+w.category,class:"note-item",onClick:F=>u(w)},[N("div",pc,[N("span",{class:re(["note-category","note-category--"+w.category])},fn((x=Re(t)[w.category])==null?void 0:x.label),3),N("span",fc,fn(w.date),1)]),N("div",mc,fn(w.title),1),w.summary?(yn(),Rn("div",yc,fn(w.summary),1)):La("",!0)],8,gc)}),128))])):(yn(),Rn("div",bc," 暂无笔记 ")),h[6]||(h[6]=N("footer",{class:"site-footer"}," 二子的知识库 · 自主学习，持续探索 ",-1))],64))])}}};ul(kc).mount("#app");
