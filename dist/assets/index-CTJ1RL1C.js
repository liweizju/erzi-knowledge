...
${u}`:u,s=s?`${s}
${A}`:A;let p=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(A,r,!0),this.lexer.state.top=p,t.length===0)break;let f=r.at(-1);if((f==null?void 0:f.type)==="code")break;if((f==null?void 0:f.type)==="blockquote"){let d=f,C=d.raw+`
`+t.join(`
`),w=this.blockquote(C);r[r.length-1]=w,i=i.substring(0,i.length-d.raw.length)+w.raw,s=s.substring(0,s.length-d.text.length)+w.text;break}else if((f==null?void 0:f.type)==="list"){let d=f,C=d.raw+`
`+t.join(`
`),w=this.list(C);r[r.length-1]=w,i=i.substring(0,i.length-f.raw.length)+w.raw,s=s.substring(0,s.length-d.raw.length)+w.raw,t=C.substring(r.at(-1).raw.length).split(`
...
`,1)[0],w=!d.trim(),M=0;if(this.options.pedantic?(M=2,f=d.trimStart()):w?M=e[1].length+1:(M=d.search(this.rules.other.nonSpaceChar),M=M>4?1:M,f=d.slice(M),M+=e[1].length),w&&this.rules.other.blankLine.test(C)&&(p+=C+`
`,n=n.substring(C.length+1),A=!0),!A){let B=this.rules.other.nextBulletRegex(M),E=this.rules.other.hrRegex(M),j=this.rules.other.fencesBeginRegex(M),R=this.rules.other.headingBeginRegex(M),Q=this.rules.other.htmlBeginRegex(M),Pn=this.rules.other.blockquoteBeginRegex(M);for(;n;){let ln=n.split(`
`,1)[0],cn;if(C=ln,this.options.pedantic?(C=C.replace(this.rules.other.listReplaceNesting,"  "),cn=C):cn=C.replace(this.rules.other.tabCharGlobal,"    "),j.test(C)||R.test(C)||Q.test(C)||Pn.test(C)||B.test(C)||E.test(C))break;if(cn.search(this.rules.other.nonSpaceChar)>=M||!C.trim())f+=`
`+cn.slice(M);else{if(w||d.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||j.test(d)||R.test(d)||E.test(d))break;f+=`
`+C}w=!C.trim(),p+=ln+`
...
`)?"":`
`)+o.raw,u.text+=`
`+o.text,this.inlineQueue.pop(),this.inlineQueue.at(-1).src=u.text):t.push(o);continue}if(e){let u="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(u);break}else throw new Error(u)}}return this.state.top=!0,t}inline(e,t=[]){return this.inlineQueue.push({src:e,tokens:t}),t}inlineTokens(e,t=[]){var l,u,A,p,f;let i=e,s=null;if(this.tokens.links){let d=Object.keys(this.tokens.links);if(d.length>0)for(;(s=this.tokenizer.rules.inline.reflinkSearch.exec(i))!=null;)d.includes(s[0].slice(s[0].lastIndexOf("[")+1,-1))&&(i=i.slice(0,s.index)+"["+"a".repeat(s[0].length-2)+"]"+i.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex))}for(;(s=this.tokenizer.rules.inline.anyPunctuation.exec(i))!=null;)i=i.slice(0,s.index)+"++"+i.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);let r;for(;(s=this.tokenizer.rules.inline.blockSkip.exec(i))!=null;)r=s[2]?s[2].length:0,i=i.slice(0,s.index+r)+"["+"a".repeat(s[0].length-r-2)+"]"+i.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);i=((u=(l=this.options.hooks)==null?void 0:l.emStrongMask)==null?void 0:u.call({lexer:this},i))??i;let a=!1,o="";for(;e;){a||(o=""),a=!1;let d;if((p=(A=this.options.extensions)==null?void 0:A.inline)!=null&&p.some(w=>(d=w.call({lexer:this},e,t))?(e=e.substring(d.raw.length),t.push(d),!0):!1))continue;if(d=this.tokenizer.escape(e)){e=e.substring(d.raw.length),t.push(d);continue}if(d=this.tokenizer.tag(e)){e=e.substring(d.raw.length),t.push(d);continue}if(d=this.tokenizer.link(e)){e=e.substring(d.raw.length),t.push(d);continue}if(d=this.tokenizer.reflink(e,this.tokens.links)){e=e.substring(d.raw.length);let w=t.at(-1);d.type==="text"&&(w==null?void 0:w.type)==="text"?(w.raw+=d.raw,w.text+=d.text):t.push(d);continue}if(d=this.tokenizer.emStrong(e,i,o)){e=e.substring(d.raw.length),t.push(d);continue}if(d=this.tokenizer.codespan(e)){e=e.substring(d.raw.length),t.push(d);continue}if(d=this.tokenizer.br(e)){e=e.substring(d.raw.length),t.push(d);continue}if(d=this.tokenizer.del(e,i,o)){e=e.substring(d.raw.length),t.push(d);continue}if(d=this.tokenizer.autolink(e)){e=e.substring(d.raw.length),t.push(d);continue}if(!this.state.inLink&&(d=this.tokenizer.url(e))){e=e.substring(d.raw.length),t.push(d);continue}let C=e;if((f=this.options.extensions)!=null&&f.startInline){let w=1/0,M=e.slice(1),B;this.options.extensions.startInline.forEach(E=>{B=E.call({lexer:this},M),typeof B=="number"&&B>=0&&(w=Math.min(w,B))}),w<1/0&&w>=0&&(C=e.substring(0,w+1))}if(d=this.tokenizer.inlineText(C)){e=e.substring(d.raw.length),d.raw.slice(-1)!=="_"&&(o=d.raw.slice(-1)),a=!0;let w=t.at(-1);(w==null?void 0:w.type)==="text"?(w.raw+=d.raw,w.text+=d.text):t.push(d);continue}if(e){let w="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(w);break}else throw new Error(w)}}return t}},ft=class{constructor(n){H(this,"options");H(this,"parser");this.options=n||Ae}space(n){return""}code({text:n,lang:e,escaped:t}){var r;let i=(r=(e||"").match(an.notSpaceStart))==null?void 0:r[0],s=n.replace(an.endingNewline,"")+`
