...
`,n=n.substring(C.length+1),A=!0),!A){let W=this.rules.other.nextBulletRegex(M),E=this.rules.other.hrRegex(M),H=this.rules.other.fencesBeginRegex(M),R=this.rules.other.headingBeginRegex(M),Y=this.rules.other.htmlBeginRegex(M),Pn=this.rules.other.blockquoteBeginRegex(M);for(;n;){let ln=n.split(`
`,1)[0],cn;if(C=ln,this.options.pedantic?(C=C.replace(this.rules.other.listReplaceNesting,"  "),cn=C):cn=C.replace(this.rules.other.tabCharGlobal,"    "),H.test(C)||R.test(C)||Y.test(C)||Pn.test(C)||W.test(C)||E.test(C))break;if(cn.search(this.rules.other.nonSpaceChar)>=M||!C.trim())I+=`
