...
`,1)[0],w=!p.trim(),M=0;if(this.options.pedantic?(M=2,g=p.trimStart()):w?M=e[1].length+1:(M=p.search(this.rules.other.nonSpaceChar),M=M>4?1:M,g=p.slice(M),M+=e[1].length),w&&this.rules.other.blankLine.test(P)&&(f+=P+`
...
`+cn.slice(M);else{if(w||p.replace(this.rules.other.tabCharGlobal,"    ").search(this.rules.other.nonSpaceChar)>=4||z.test(p)||R.test(p)||_.test(p))break;g+=`
`+P}w=!P.trim(),f+=an+`
...
`,source:""}],categories:{tech:{label:"技术前沿",color:"#60a5fa",priority:1},inspiration:{label:"灵感采集",color:"#a78bfa",priority:2},reading:{label:"知识阅读",color:"#34d399",priority:3},reflection:{label:"反思与整理",color:"#f09383",priority:4},diary:{label:"二子日记",color:"#e879a0",priority:5}}},ic={class:"container"},sc={class:"site-header"},rc={class:"stats"},oc={class:"stat"},lc={class:"stat-num"},ac={class:"stat"},cc={class:"stat-num"},uc={class:"stat"},hc={class:"stat-num"},dc={class:"filter-bar"},pc=["onClick"],fc={key:0,class:"note-list"},Ac=["onClick"],gc={class:"note-meta"},Ic={class:"note-date"},mc={class:"note-title"},bc={key:0,class:"note-summary"},yc={key:1,class:"empty-state"},wc={class:"detail-header"},vc={class:"detail-title"},Sc={class:"detail-meta"},kc={class:"note-date"},xc=["innerHTML"],Pc={__name:"App",setup(n){N.setOptions({breaks:!1,gfm:!0});const e=ws.notes,t=ws.categories,i=Et(null),s=Et(null),r=Et("加载中..."),l=ot(()=>new Set(e.map(p=>p.date)).size),o=ot(()=>i.value?e.filter(g=>g.category===i.value):e),a=ot(()=>s.value?N(s.value.content):"");async function u(){try{const g=localStorage.getItem("erzi-knowledge-visits");g?r.value=parseInt(g):r.value=1;const p=(parseInt(g)||0)+1;localStorage.setItem("erzi-knowledge-visits",p.toString()),r.value=p}catch(g){console.error("访问统计加载失败:",g),r.value="统计不可用"}}ir(()=>{u()});function h(g){s.value=g,Kt(()=>window.scrollTo(0,0))}function f(){s.value=null,Kt(()=>window.scrollTo(0,0))}return(g,p)=>{var P;return bn(),Rn("div",ic,[s.value?(bn(),Rn(An,{key:1},[F("div",wc,[F("button",{class:"back-btn",onClick:f},[...p[7]||(p[7]=[F("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[F("polyline",{points:"15 18 9 12 15 6"})],-1),fe(" 返回 ",-1)])]),F("h1",vc,In(s.value.title),1),F("div",Sc,[F("span",{class:oe(["note-category","note-category--"+s.value.category])},In((P=Re(t)[s.value.category])==null?void 0:P.label),3),F("span",kc,In(s.value.date),1)])]),F("div",{class:"note-content",innerHTML:a.value},null,8,xc),F("footer",{class:"site-footer"},[F("button",{class:"back-btn",onClick:f},[...p[8]||(p[8]=[F("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[F("polyline",{points:"15 18 9 12 15 6"})],-1),fe(" 返回列表 ",-1)])])])],64)):(bn(),Rn(An,{key:0},[F("header",sc,[p[4]||(p[4]=F("div",{class:"site-title"},"二子的知识库",-1)),p[5]||(p[5]=F("div",{class:"site-subtitle"},"跟二子一起学习",-1)),F("div",rc,[F("span",oc,[F("span",lc,In(Re(e).length),1),p[1]||(p[1]=fe(" 篇笔记",-1))]),F("span",ac,[F("span",cc,In(l.value),1),p[2]||(p[2]=fe(" 天探索",-1))]),F("span",uc,[F("span",hc,In(r.value),1),p[3]||(p[3]=fe(" 次访问",-1))])])]),F("div",dc,[F("button",{class:oe(["filter-btn",{active:i.value===null}]),onClick:p[0]||(p[0]=w=>i.value=null)},"全部",2),(bn(!0),Rn(An,null,Hi(Re(t),(w,M)=>(bn(),Rn("button",{key:M,class:oe(["filter-btn",{active:i.value===M}]),onClick:B=>i.value=M},In(w.label),11,pc))),128))]),o.value.length?(bn(),Rn("div",fc,[(bn(!0),Rn(An,null,Hi(o.value,w=>{var M;return bn(),Rn("div",{key:w.id+w.category,class:"note-item",onClick:B=>h(w)},[F("div",gc,[F("span",{class:oe(["note-category","note-category--"+w.category])},In((M=Re(t)[w.category])==null?void 0:M.label),3),F("span",Ic,In(w.date),1)]),F("div",mc,In(w.title),1),w.summary?(bn(),Rn("div",bc,In(w.summary),1)):Dl("",!0)],8,Ac)}),128))])):(bn(),Rn("div",yc," 暂无笔记 ")),p[6]||(p[6]=F("footer",{class:"site-footer"}," 二子的知识库 · 自主学习，持续探索 ",-1))],64))])}}};ha(Pc).mount("#app");
