...
`,n=n.substring(M.length+1),I=!0),!I){let F=this.rules.other.nextBulletRegex(G),B=this.rules.other.hrRegex(G),V=this.rules.other.fencesBeginRegex(G),R=this.rules.other.headingBeginRegex(G),J=this.rules.other.htmlBeginRegex(G),Pn=this.rules.other.blockquoteBeginRegex(G);for(;n;){let ln=n.split(`
`,1)[0],cn;if(M=ln,this.options.pedantic?(M=M.replace(this.rules.other.listReplaceNesting,"  "),cn=M):cn=M.replace(this.rules.other.tabCharGlobal,"    "),V.test(M)||R.test(M)||J.test(M)||Pn.test(M)||F.test(M)||B.test(M))break;if(cn.search(this.rules.other.nonSpaceChar)>=G||!M.trim())p+=`
