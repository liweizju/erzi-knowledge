...
${u}`:u,i=i?`${i}
${h}`:h;let d=this.lexer.state.top;if(this.lexer.state.top=!0,this.lexer.blockTokens(h,r,!0),this.lexer.state.top=d,t.length===0)break;let A=r.at(-1);if((A==null?void 0:A.type)==="code")break;if((A==null?void 0:A.type)==="blockquote"){let f=A,P=f.raw+`
`+t.join(`
`),w=this.blockquote(P);r[r.length-1]=w,s=s.substring(0,s.length-f.raw.length)+w.raw,i=i.substring(0,i.length-f.text.length)+w.text;break}else if((A==null?void 0:A.type)==="list"){let f=A,P=f.raw+`
`+t.join(`
`),w=this.list(P);r[r.length-1]=w,s=s.substring(0,s.length-A.raw.length)+w.raw,i=i.substring(0,i.length-f.raw.length)+w.raw,t=P.substring(r.at(-1).raw.length).split(`
...
`,1)[0],w=!f.trim(),M=0;if(this.options.pedantic?(M=2,A=f.trimStart()):w?M=e[1].length+1:(M=f.search(this.rules.other.nonSpaceChar),M=M>4?1:M,A=f.slice(M),M+=e[1].length),w&&this.rules.other.blankLine.test(P)&&(d+=P+`
...
`)?"":`
`)+o.raw,u.text+=`
`+o.text,this.inlineQueue.pop(),this.inlineQueue.at(-1).src=u.text):t.push(o);continue}if(e){let u="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(u);break}else throw new Error(u)}}return this.state.top=!0,t}inline(e,t=[]){return this.inlineQueue.push({src:e,tokens:t}),t}inlineTokens(e,t=[]){var a,u,h,d,A;let s=e,i=null;if(this.tokens.links){let f=Object.keys(this.tokens.links);if(f.length>0)for(;(i=this.tokenizer.rules.inline.reflinkSearch.exec(s))!=null;)f.includes(i[0].slice(i[0].lastIndexOf("[")+1,-1))&&(s=s.slice(0,i.index)+"["+"a".repeat(i[0].length-2)+"]"+s.slice(this.tokenizer.rules.inline.reflinkSearch.lastIndex))}for(;(i=this.tokenizer.rules.inline.anyPunctuation.exec(s))!=null;)s=s.slice(0,i.index)+"++"+s.slice(this.tokenizer.rules.inline.anyPunctuation.lastIndex);let r;for(;(i=this.tokenizer.rules.inline.blockSkip.exec(s))!=null;)r=i[2]?i[2].length:0,s=s.slice(0,i.index+r)+"["+"a".repeat(i[0].length-r-2)+"]"+s.slice(this.tokenizer.rules.inline.blockSkip.lastIndex);s=((u=(a=this.options.hooks)==null?void 0:a.emStrongMask)==null?void 0:u.call({lexer:this},s))??s;let l=!1,o="";for(;e;){l||(o=""),l=!1;let f;if((d=(h=this.options.extensions)==null?void 0:h.inline)!=null&&d.some(w=>(f=w.call({lexer:this},e,t))?(e=e.substring(f.raw.length),t.push(f),!0):!1))continue;if(f=this.tokenizer.escape(e)){e=e.substring(f.raw.length),t.push(f);continue}if(f=this.tokenizer.tag(e)){e=e.substring(f.raw.length),t.push(f);continue}if(f=this.tokenizer.link(e)){e=e.substring(f.raw.length),t.push(f);continue}if(f=this.tokenizer.reflink(e,this.tokens.links)){e=e.substring(f.raw.length);let w=t.at(-1);f.type==="text"&&(w==null?void 0:w.type)==="text"?(w.raw+=f.raw,w.text+=f.text):t.push(f);continue}if(f=this.tokenizer.emStrong(e,s,o)){e=e.substring(f.raw.length),t.push(f);continue}if(f=this.tokenizer.codespan(e)){e=e.substring(f.raw.length),t.push(f);continue}if(f=this.tokenizer.br(e)){e=e.substring(f.raw.length),t.push(f);continue}if(f=this.tokenizer.del(e,s,o)){e=e.substring(f.raw.length),t.push(f);continue}if(f=this.tokenizer.autolink(e)){e=e.substring(f.raw.length),t.push(f);continue}if(!this.state.inLink&&(f=this.tokenizer.url(e))){e=e.substring(f.raw.length),t.push(f);continue}let P=e;if((A=this.options.extensions)!=null&&A.startInline){let w=1/0,M=e.slice(1),B;this.options.extensions.startInline.forEach(_=>{B=_.call({lexer:this},M),typeof B=="number"&&B>=0&&(w=Math.min(w,B))}),w<1/0&&w>=0&&(P=e.substring(0,w+1))}if(f=this.tokenizer.inlineText(P)){e=e.substring(f.raw.length),f.raw.slice(-1)!=="_"&&(o=f.raw.slice(-1)),l=!0;let w=t.at(-1);(w==null?void 0:w.type)==="text"?(w.raw+=f.raw,w.text+=f.text):t.push(f);continue}if(e){let w="Infinite loop on byte: "+e.charCodeAt(0);if(this.options.silent){console.error(w);break}else throw new Error(w)}}return t}},At=class{constructor(n){j(this,"options");j(this,"parser");this.options=n||he}space(n){return""}code({text:n,lang:e,escaped:t}){var r;let s=(r=(e||"").match(ln.notSpaceStart))==null?void 0:r[0],i=n.replace(ln.endingNewline,"")+`
...
`,source:""}],categories:{tech:{label:"技术前沿",color:"#60a5fa",priority:1},inspiration:{label:"灵感采集",color:"#a78bfa",priority:2},reading:{label:"知识阅读",color:"#34d399",priority:3},reflection:{label:"反思与整理",color:"#f09383",priority:4},diary:{label:"二子日记",color:"#e879a0",priority:5}}},sc={class:"container"},ic={class:"site-header"},rc={class:"stats"},oc={class:"stat"},lc={class:"stat-num"},ac={class:"stat"},cc={class:"stat-num"},uc={class:"stat"},hc={class:"stat-num"},pc={class:"filter-bar"},fc=["onClick"],dc={key:0,class:"note-list"},gc=["onClick"],Ac={class:"note-meta"},Ic={class:"note-date"},mc={class:"note-title"},bc={key:0,class:"note-summary"},yc={key:1,class:"empty-state"},wc={class:"detail-header"},vc={class:"detail-title"},Sc={class:"detail-meta"},kc={class:"note-date"},xc=["innerHTML"],Pc={__name:"App",setup(n){N.setOptions({breaks:!1,gfm:!0});const e=wi.notes,t=wi.categories,s=Et(null),i=Et(null),r=Et("加载中..."),l=ot(()=>new Set(e.map(f=>f.date)).size),o=ot(()=>s.value?e.filter(A=>A.category===s.value):e),a=ot(()=>i.value?N(i.value.content):"");async function u(){try{const A=localStorage.getItem("erzi-knowledge-visits");A?r.value=parseInt(A):r.value=1;const f=(parseInt(A)||0)+1;localStorage.setItem("erzi-knowledge-visits",f.toString()),r.value=f}catch(A){console.error("访问统计加载失败:",A),r.value="统计不可用"}}sr(()=>{u()});function h(A){i.value=A,Kt(()=>window.scrollTo(0,0))}function d(){i.value=null,Kt(()=>window.scrollTo(0,0))}return(A,f)=>{var P;return bn(),Rn("div",sc,[i.value?(bn(),Rn(gn,{key:1},[F("div",wc,[F("button",{class:"back-btn",onClick:d},[...f[7]||(f[7]=[F("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[F("polyline",{points:"15 18 9 12 15 6"})],-1),de(" 返回 ",-1)])]),F("h1",vc,In(i.value.title),1),F("div",Sc,[F("span",{class:oe(["note-category","note-category--"+i.value.category])},In((P=Re(t)[i.value.category])==null?void 0:P.label),3),F("span",kc,In(i.value.date),1)])]),F("div",{class:"note-content",innerHTML:a.value},null,8,xc),F("footer",{class:"site-footer"},[F("button",{class:"back-btn",onClick:d},[...f[8]||(f[8]=[F("svg",{viewBox:"0 0 24 24",fill:"none",stroke:"currentColor","stroke-width":"2","stroke-linecap":"round","stroke-linejoin":"round"},[F("polyline",{points:"15 18 9 12 15 6"})],-1),de(" 返回列表 ",-1)])])])],64)):(bn(),Rn(gn,{key:0},[F("header",ic,[f[4]||(f[4]=F("div",{class:"site-title"},"二子的知识库",-1)),f[5]||(f[5]=F("div",{class:"site-subtitle"},"跟二子一起学习",-1)),F("div",rc,[F("span",oc,[F("span",lc,In(Re(e).length),1),f[1]||(f[1]=de(" 篇笔记",-1))]),F("span",ac,[F("span",cc,In(l.value),1),f[2]||(f[2]=de(" 天探索",-1))]),F("span",uc,[F("span",hc,In(r.value),1),f[3]||(f[3]=de(" 次访问",-1))])])]),F("div",pc,[F("button",{class:oe(["filter-btn",{active:s.value===null}]),onClick:f[0]||(f[0]=w=>s.value=null)},"全部",2),(bn(!0),Rn(gn,null,Hs(Re(t),(w,M)=>(bn(),Rn("button",{key:M,class:oe(["filter-btn",{active:s.value===M}]),onClick:B=>s.value=M},In(w.label),11,fc))),128))]),o.value.length?(bn(),Rn("div",dc,[(bn(!0),Rn(gn,null,Hs(o.value,w=>{var M;return bn(),Rn("div",{key:w.id+w.category,class:"note-item",onClick:B=>h(w)},[F("div",Ac,[F("span",{class:oe(["note-category","note-category--"+w.category])},In((M=Re(t)[w.category])==null?void 0:M.label),3),F("span",Ic,In(w.date),1)]),F("div",mc,In(w.title),1),w.summary?(bn(),Rn("div",bc,In(w.summary),1)):Dl("",!0)],8,gc)}),128))])):(bn(),Rn("div",yc," 暂无笔记 ")),f[6]||(f[6]=F("footer",{class:"site-footer"}," 二子的知识库 · 自主学习，持续探索 ",-1))],64))])}}};ha(Pc).mount("#app");
