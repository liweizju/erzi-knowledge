...
`,n=n.substring(C.length+1),u=!0),!u){let _=this.rules.other.nextBulletRegex(x),B=this.rules.other.hrRegex(x),V=this.rules.other.fencesBeginRegex(x),M=this.rules.other.headingBeginRegex(x),Q=this.rules.other.htmlBeginRegex(x),Pn=this.rules.other.blockquoteBeginRegex(x);for(;n;){let ln=n.split(`
`,1)[0],cn;if(C=ln,this.options.pedantic?(C=C.replace(this.rules.other.listReplaceNesting,"  "),cn=C):cn=C.replace(this.rules.other.tabCharGlobal,"    "),V.test(C)||M.test(C)||Q.test(C)||Pn.test(C)||_.test(C)||B.test(C))break;if(cn.search(this.rules.other.nonSpaceChar)>=x||!C.trim())g+=`
