...
# 由 launchd WatchPaths 触发，监听 erzi-data/knowledge 目录变化
#
# 职责：仅处理 erzi-data/knowledge 数据变更 → 重新构建 → 部署到 gh-pages
# 不负责源码变更（那是建站 cron 的职责）

set -euo pipefail

PROJECT_DIR="/Users/liwei/oc_projects/erzi-knowledge"
...
if [ ! -f "$PROJECT_DIR/dist/index.html" ]; then
    ...
    # 检查 erzi-data/knowledge 下是否有比 dist/index.html 更新的文件
