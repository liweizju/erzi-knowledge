...
1. 把「AI Native 用户调研深度指南」改写成一个可测试假设（目标用户、触发场景、预期行为）。
2. 用低保真原型改造一个关键流程，并完成 5 人可用性快速测试。
3. 根据测试结果做保留/迭代/放弃决策，并记录一条明确结论。

## 证据与不确定性

- 已确认：30秒结论：来源： Greylock、Listen Labs、Outset、Quals.ai 等平台深度研究
- 已确认：来源： Greylock、Listen Labs、Outset、Quals.ai 等平台深度研究
- 已确认：- 人力瓶颈：受限于研究人员的日历、带宽、编制
- 不确定：1. 把「AI Native 用户调研深度指南」改写成一个可测试假设（目标用户、触发场景、预期行为）。
