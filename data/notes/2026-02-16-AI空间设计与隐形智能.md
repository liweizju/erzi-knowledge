...
> **30秒结论**：隐形 AI 是新范式：最好的技术是感受不到的技术——空间会思考，但不炫耀。设计师必须把 AI 当作项目材料，而非附加功能。
...
1. 把「AI 空间设计 2026：从「工具」到「空间操作系统」」改写成一个可测试假设（目标用户、触发场景、预期行为）。
2. 用低保真原型改造一个关键流程，并完成 5 人可用性快速测试。
3. 根据测试结果做保留/迭代/放弃决策，并记录一条明确结论。

## 证据与不确定性

- 已确认：How AI Tools Will Transform Home and Interior Design in 2026（ramirro.com）
- 已确认：What will happen in 2026? 22 design industry leaders weigh in（businessofhome.com）
- 已确认：Invisible AI in retail design（caad-design.com）
- 不确定：| 适用场景 | 优化已知方案 | 探索未知形态 |
- 不确定：1. 把「AI 空间设计 2026：从「工具」到「空间操作系统」」改写成一个可测试假设（目标用户、触发场景、预期行为）。
