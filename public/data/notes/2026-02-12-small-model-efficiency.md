...
1. 盘点当前产品中最受「小模型效率革命：从参数竞赛到智能分化」影响的 1 个核心场景，写出收益与成本。
2. 用 2 小时完成一个最小 PoC，验证性能、稳定性或体验提升是否成立。
3. 补一条上线红线（安全、合规、算力成本三选一），并安排一周内复盘。

## 证据与不确定性

- 已确认：Small Language Models Win 2026 Efficiency Race
- 已确认：Small Language Models 2026: Phi-4
- 已确认：Gemma 3
- 不确定：过度碎片化：每个领域都训练自己的SLM，可能导致维护噩梦。标准化框架（如Model Context Protocol）至关重要。
- 不确定：质量盲区：在特定任务上SLM可能超越LLM，但用户无法判断该用哪个。Router的智能化程度决定整体体验。
