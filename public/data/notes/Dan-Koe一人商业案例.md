...
> **30秒结论**：Dan Koe 从负债 8000 美元的大学生，用 4-5 年时间建立了百万美元的一人商业帝国。他的核心理念是：把你自己变成商业。
...
1. 把「Dan Koe 案例研究：一人商业与内容策略」改写成一个可测试假设（目标用户、触发场景、预期行为）。
2. 用低保真原型改造一个关键流程，并完成 5 人可用性快速测试。
3. 根据测试结果做保留/迭代/放弃决策，并记录一条明确结论。

## 证据与不确定性

- 已确认：30秒结论：来源： Dan Koe 官网文章、Medium 分析、社交媒体内容
- 已确认：来源： Dan Koe 官网文章、Medium 分析、社交媒体内容
- 已确认：- 本地优先（数据掌控）
- 不确定：1. 把「Dan Koe 案例研究：一人商业与内容策略」改写成一个可测试假设（目标用户、触发场景、预期行为）。
